...
# Namespace for the Lich 5 scripting engine, which provides Ruby automation
# for the text-based games GemStone IV and DragonRealms.
module Lich
  # Namespace for GemStone IV combat and game data definitions.
  module Gemstone
    # Namespace for combat-related patterns and event definitions.
    module Combat
      # Namespace for attack, spell, and combat-action pattern definitions.
      module Definitions
        # Attack pattern definitions for parsing combat log messages in GemStone IV.
        #
        # This module defines all recognized attack types (melee, ranged, maneuvers,
        # spells, companion/pet actions, and environmental damage) as regular expressions.
        # Attack definitions are organized by priority and category (PRIORITY_ATTACKS,
        # BASIC_ATTACKS, WEAPON_ATTACKS, MANEUVER_ATTACKS, etc.) and concatenated in
        # ALL_ATTACKS with load-bearing order: priority defs preempt generic patterns,
        # and second-person patterns are matched before third-person to avoid ambiguity.
        #
        # Each attack is defined as an AttackDef Struct with a symbolic name and an
        # array of Regexp patterns that capture attacker, target, and weapon details.
        # Named capture groups (.+?, [^!]+, etc.) bind these extracted values to the
        # parsed event.
        #
        # The module also provides prefix-detection methods for modifier lines (ambush
        # and redirect prefixes) that affect how attacks are classified, and gate-based
        ...
        module Attacks
          ...
          # Detects whether a combat log line describes environmental damage or a
          # weapon recoil that the parser should report as inbound (damage taken by US)
          # without requiring an explicit attacker.
          #
          # Environmental ticks (frigid wind, thorn recoil) do not include creature links
          # and are normally filtered by the chunk gate; this method lets the gate pass
          # such lines so they reach the parser with the correct ledger classification.
          #
          # @param line [String] a raw combat log line
          # @return [Boolean] true if the line matches a self-inflicted attack pattern
          # @example
          #   Attacks.self_inflicted_line?("The burn of the cold tears precious warmth from your flesh.") #=> true
          #   Attacks.self_inflicted_line?("You swing a longsword at a goblin!") #=> false
          # @api private
