...
module Lich
  ...
  module Gemstone
    # Namespace for combat observation and analysis.
    module Combat
      # Persists the combat observer event stream to a SQLite database.
      #
      # Maintains a relational schema of sessions, creatures, attacks, resolutions,
      # flares, hits, and statuses. Handles both live auto-sessioned recording and
      # explicit session control for replay/headless analysis. Manages spawn-tree
      # linkage (root/parent attack relationships), creature lifecycle tracking
      # (kills, wounds), and attack attribution of status effects via a window
      # heuristic.
      #
      # Thread-safe: serializes all public entry points on a non-reentrant mutex
      # to coordinate between the AsyncProcessor worker thread (record) and the
      # consuming script's event loop (check_idle!, close, start_session).
      #
      # @example Record a live hunt
      #   rec = Combat::Recorder.new('/tmp/combat.db', character: Char.name, idle_timeout: 300)
      #   rec.subscribe!
      #   loop { rec.check_idle! }
      # @example Explicit session (replay)
      #   rec = Combat::Recorder.new('/tmp/combat.db')
      #   rec.start_session(character: 'Nisugi', source: 'log.txt', at: t0)
      #   feed_events_to(rec)
      #   rec.finish_session(at: t1)
      class Recorder
        HANDLER_NAME = 'combat_recorder'

        # SQLite schema: seven-table relational design for combat events.
        #
        # Tables: sessions (hunt unit), creatures (subject registry), attacks
        # (swing/cast/inbound), resolutions (roll facts), flares (proc damage),
        # hits (damage events with crit denormalization), statuses (effect stream).
        #
        # Sources of truth: attacks/resolutions/flares/hits unpack the :attack
        # payload only (derived :damage/:wound/:fatal_crit/:amputation events are
        # not subscribed). Statuses come from :status/:stun/:roundtime/:spell_loss
        # /:ucs emits only.
        #
        # Spawn-tree closure: root_attack_id points every event at the initiating
        # shot; parent_attack_id/parent_confidence record the immediate spawner
        # only when game-declared (blink bracket) - ambiguous echoes are unparented.
        #
        # @see #initialize
        # @see #start_session
        # @see #record
        ...
        # Open a new session, finishing any open session first.
        #
        # Acquires the serialization mutex; calls the internal _locked variant.
        # Sessions record hunts bounded by a 5-minute gap without combat events
        # (inbound or outbound). With idle_timeout set at initialization, the
        # recorder opens lazily on the first event and closes once that gap elapses.
        # For explicit control (e.g., replay), omit idle_timeout and call this and
        # finish_session directly.
        #
        # @param character [String, nil] character name, defaults to nil
        # @param source [String, nil] origin description ("live", log file path, etc.),
        #   defaults to nil
        # @param at [Time] timestamp for session start, defaults to Time.now
        # @return [Integer] the row ID of the newly created session
        # @example Start a replay session
        #   rec.start_session(character: 'Nisugi', source: '/logs/hunt.txt', at: Time.at(1000))
        ...
        # @api private
        # Internal variant of start_session; does not acquire the mutex.
        #
        # Assumes the caller already holds the lock. Closes any existing session,
        # inserts a new session row, resets sequence counter and creature cache,
        # and returns the new session ID.
        #
        # @param character [String, nil] character name
        # @param source [String, nil] origin description
        # @param at [Time] timestamp for session start
        # @return [Integer] the new session ID
        ...
        # @api private
        # Internal variant of finish_session; does not acquire the mutex.
        #
        # Assumes the caller already holds the lock. Updates the current session's
        # ended_at timestamp and clears session and attack state. Returns the closed
        # session's ID, or nil if no session was open.
        #
        # @param at [Time] timestamp for session end, defaults to Time.now
        # @return [Integer, nil] the closed session ID, or nil if no session was open
        ...
        # Unregister the named handler from the observer feed.
        #
        # Stops listening to :attack, :status, :stun, :roundtime, :spell_loss, :ucs
        # events. Safe to call when not subscribed.
        #
        # @return [void]
        def unsubscribe!
        ...
        # @api private
        # Internal variant of check_idle!; does not acquire the mutex.
        #
        # Assumes the caller already holds the lock. Finishes the current session
        # if idle_timeout was set, a session is open, an event has been recorded,
        # and the gap since the last event exceeds idle_timeout. Uses the last
        # event's timestamp as the session end (town time never pads a hunt).
        #
        # @param now [Time] current time for comparison, defaults to Time.now
        # @return [void]
        ...
        # Record a combat event from the observer feed.
        #
        # Public entry point called by the named observer handler. Acquires the
        # serialization mutex and dispatches to record_locked. Rescue catches and
        # logs any error so a payload bug never crashes the processor.
        #
        # @param type [Symbol] event type: :attack, :status, :stun, :roundtime,
        #   :spell_loss, or :ucs
        # @param data [Hash] event payload
        # @return [void]
        # @note Errors are logged via Lich.log (if available) and never re-raised
        def record(type, data)
          @mutex.synchronize { record_locked(type, data) }
        end

        # @api private
        # Internal variant of record; does not acquire the mutex.
        #
        # Assumes the caller already holds the lock. If idle_timeout is set,
        # checks for session expiry, auto-opens a new session if needed, and
        # updates the last-event timestamp. Then dispatches by type to the
        # appropriate handler (record_attack or record_status variants).
        # Returns early if no session is open.
        #
        # @param type [Symbol] event type: :attack, :status, :stun, :roundtime,
        #   :spell_loss, or :ucs
        # @param data [Hash] event payload
