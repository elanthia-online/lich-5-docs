...
#
# Lich 5 is a Ruby scripting system for the text-based games GemStone IV and
# DragonRealms, providing game automation, data tracking, and combat analysis.
module Lich
  # Namespace for GemStone IV–specific functionality.
  module Gemstone
    # Namespace for combat event processing and analysis.
    module Combat
      # State machine–based parser for combat events from the game feed.
      #
      # Converts chunks of game lines into structured combat events (attacks, flares,
      # damage, crits, outcomes). Events are linked into a spawn tree (blink brackets,
      # echo flares), attributed to creatures, and persisted to the creature registry
      # and observer feed. Handles multi-target attacks (volley, AoE), ambushes,
      # redirects, and trailing-effect DoT ticks.
      #
      # The parser runs after each game prompt; events are emitted to subscribers
      # (recorders, damage trackers, etc.) before creature state is mutated.
      module Processor
        ...
        # Emits all parse-phase facts queued by emit_fact, then clears the queue.
        #
        # Facts (:status, :ucs, :spell_loss) are queued while a chunk parses and flushed
        # after all its :attack emits have completed. This ensures the attack event
        # arrives before the facts that describe its effects.
        #
        # @return [void]
        # @api private
        ...
        # Matches the server epoch timestamp in a <prompt time="..."> tag.
        #
        # Extracts the raw seconds since the server epoch (a Unix-like timestamp in
        # the game server's timezone). The prompt_time method uses this to anchor
        # duration estimates (stun expiry, status timestamps) to a known moment.
        #
        # @example
        #   "<prompt time=\"1694560800\">" =~ PROMPT_TIME_PATTERN #=> $1 == "1694560800"
        #
        # @see #prompt_time XMLData.server_time_offset
        ...
        # Marks a creature ID for death-state monitoring across future chunks.
        #
        # When an attack lands, the creature's death flag may not yet be set in the
        # registry (the async game feed lags). This method queues the creature for
        # sweep_death_watch to poll in the next few chunks. If the dead flag appears,
        # a :status dead (add) emit fires once per creature.
        #
        # @param id [Integer, String, nil] the creature's ID; nil calls are a no-op
        # @return [void]
        # @api private
        #
        # @see #sweep_death_watch DEATH_WATCH_SWEEPS
        ...
        # Checks whether a creature is flagged as dead in the registry.
        #
        # Consults the creature's crtr_flag?(:dead) or has_status?('dead') to
        # determine its state. Returns false if the creature does not respond to
        # these methods or if any exception is raised during the check.
        #
        # @param creature [Object] a creature instance from the Creature registry
        # @return [Boolean] true if the creature is marked dead; false otherwise
        # @api private
