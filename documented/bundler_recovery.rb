...
module Lich
  ...
  class BundlerRecovery
    ...
    class << self
      ...
      #
      # Gracefully handles missing or unparseable payload data by falling back to a default
      # temporary directory. If logging fails, the error is silently ignored.
      #
      # @param payload_or_path [Hash, String] the parsed JSON payload (Hash) or path to the payload file (String)
      # @param error [StandardError] the error raised by the helper
      # @return [void]
      # @api private
    ...
    #
    # @param lich_dir [String] the path to the Lich installation directory
    # @param gem_home [String] the target gem home directory (defaults to Gem.dir)
    # @param helper_launcher [Proc, nil] a callable for launching the macOS helper process; if nil, uses {#launch_macos_helper}
    # @param installer [Proc, nil] a callable for installing gems; if nil, uses {#install_gem}
    # @return [void]
    ...
    #
    # Checks for macOS runtime support, required files and executables, and (when promoting
    # native gems) development headers and build tools. Returns an error message if any
    # check fails, or nil if all checks pass.
    #
    # @param missing [String, Array<String>] the gem name(s) being recovered
