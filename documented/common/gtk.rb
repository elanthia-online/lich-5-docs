    ...
    if defined?(Gtk)
      unless defined?(Lich::Common::GtkSignalHandlerRetention)
        ...
        module GtkSignalHandlerRetention
          ...
          #
          # ruby-gnome stores signal handlers in C-side closures that Ruby's GC cannot
          # see, which can leave long-lived GTK callbacks eligible for collection. This
          # method wraps the native {signal_connect} to retain both the receiver wrapper
          # and the callback Proc on the Ruby side, ensuring they survive until the
          # receiver is destroyed.
          #
          # The first call to signal_connect on a receiver also installs a 'destroy' signal
          # handler that automatically cleans up retained references when the widget is destroyed.
          #
          # @param signal [String] the GTK signal name, e.g. "clicked"
          # @param args [Array] additional arguments passed to the native signal_connect
          # @yield [Object] the callback block, which receives signal-specific arguments
          # @return [Integer, Object] the signal handler ID from the native GTK layer
          # @raise [StandardError] if the native signal connection fails
          # @note Callbacks are retained in {Lich::Common.gtk_signal_handlers} until the
          #   receiver is destroyed or the handler is manually disconnected
          # @see Lich::Common.gtk_signal_handlers
          # @see Lich::Common.release_gtk_signal_handlers
      ...
      unless defined?(Lich::Common::GtkIdleRetention)
        ...
        module GtkIdleRetention
          ...
          #
          # GLib idle callbacks are stored in C-side structures that Ruby's GC cannot see.
          # This method wraps {GLib::Idle.add} to retain the callback Proc on the Ruby side,
          # keeping it alive until the callback stops repeating or raises an exception.
          #
          # @param args [Array] arguments passed to GLib::Idle.add
          # @yield [Object] the callback block, invoked when the main loop is idle; return false to stop
          # @return [Integer, nil] the idle source ID if successfully queued, or nil on failure
          # @note Callbacks are retained in {Lich::Common.gtk_idle_callbacks} and automatically
          #   removed when they stop repeating (return false), raise, or are otherwise cleaned up
          # @see Lich::Common.gtk_idle_callbacks
          # @see Lich::Common.with_gtk_registry_lock
          def add(*args, &block)
            return super(*args, &block) unless block
