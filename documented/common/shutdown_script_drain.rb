...
#
# Lich provides a text-based scripting interface for GemStone IV and DragonRealms,
# allowing script authors to write .lic files that interact with game servers through
# a managed execution environment.
module Lich
  # Namespace for shared utilities and common functionality across Lich 5.
  module Common
    ...
    module ShutdownScriptDrain
      ...
      # Registers newly-observed scripts and enqueues them for shutdown kill.
      #
      # Called during shutdown drain to detect scripts spawned after the initial shutdown
      # phase began. Mutates the scripts array by appending new entries and enqueues each
      # for termination.
      #
      # @param scripts [Array<#kill,#name>] accumulating list of scripts to terminate; mutated in place
      # @param observed [Array<#kill,#name>] scripts currently registered as alive
      # @return [Array<#kill,#name>] newly-observed scripts that were not in the scripts list
      # @api private
