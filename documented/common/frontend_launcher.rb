...
module Lich
  # Namespace for frontend launcher and discovery utilities shared across Lich.
  module Common
    ...
    module FrontendLauncher
      SpawnPlan = Struct.new(:environment, :argv, keyword_init: true) do
        # Freezes the environment and argv into an immutable spawn plan.
        #
        # @param environment [Hash] environment variables for the spawned process
        # @param argv [Array<String>] command and arguments for the spawned process
        # @return [void]
      ...
      # Base exception for all frontend launcher errors.
      class Error < StandardError; end
      # Raised when a frontend launcher adapter or platform configuration is not supported.
      class UnsupportedError < Error; end
      # Raised when a required frontend executable or launcher is not installed or not found.
      class UnavailableError < Error; end

      # Launch data field names that are local to the Lich process and must be
      # removed before the data is serialized for a native frontend via Simutronics'
      # .sal contract.
      #
      # @see #native_session_data
      PROCESS_LOCAL_LAUNCH_FIELDS = %w[FRONTEND CUSTOMLAUNCHARGV].freeze
      # Regular expression matching Lich's connection-configuration placeholder tokens.
      # These tokens are replaced with the actual connection values after the local
      # proxy listener is opened.
      #
      # @example
      #   "%host%" matches when rendering a connection
      #   "%port%" matches when rendering a connection
      #   "%key%" matches when rendering a connection
      # @see .render_connection
      # @see .spawn_plan
