...
module Lich
  # Namespace for common functionality shared across Lich5 components.
  module Common
    # Handles authentication to game servers with automatic provider fallback and retry logic.
    #
    # Supports two authentication providers: EAccess (socket-based, eaccess.play.net:7910)
    # and WebLogin (HTTPS-based). By default, EAccess is attempted first; if it fails for
    # transient reasons (connection errors, timeouts), WebLogin is automatically used as a
    # fallback for single-character logins. Fatal auth failures (bad credentials, invalid
    # account) are never retried or fallen back on.
    #
    # @see EAccess
    # @see WebLogin
