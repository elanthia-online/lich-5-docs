...
module Lich
  module Common
    ...
    module BindHostResolver
      ...
      #
      # @param address_list [Array<Addrinfo>] local addresses to search
      # @return [Resolution] the Tailscale address with no warning
      # @raise [Error] if no address in the 100.64.0.0/10 range is found
      # @api private
      ...
      #
      # Probes the default route to identify the preferred interface, then searches
      # for a private IPv4 address in RFC1918 ranges (192.168/16, 10/8, 172.16/12)
      # in preference order. Returns with a warning about network exposure.
      #
      # @param address_list [Array<Addrinfo>] local addresses to search
      # @param route_probe [#call] callable returning the default-route source address or nil
      # @return [Resolution] a private IPv4 address with a user-facing security warning
      # @raise [Error] if no private IPv4 address is found
      # @api private
      ...
      #
      # Returns a warning string if the address looks reachable from untrusted networks.
      # Loopback, private, and Tailscale addresses pass through with no warning.
      # Non-IP hostnames return nil (cannot judge without resolution).
      #
      # @param token [String] an IP address or hostname
      # @return [String, nil] a warning message, or nil if safe
      # @api private
      ...
      #
      # @param address_list [Array<Addrinfo>] local addresses
      # @return [Array<String>] unique IPv4 address strings
      # @api private
      ...
      #
      # @param address [String] an IP address
      # @return [Boolean] true if the address is private, false otherwise or if unparseable
      # @api private
