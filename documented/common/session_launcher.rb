...
module Lich
  # Namespace for common utilities and helpers shared across Lich.
  module Common
    # Launches independent child Lich sessions using prepared launch_data
    # mapped to CLI-style argv.
    module SessionLauncher
      # Directory path overrides that child Lich processes may inherit or explicitly override.
      #
      # Each entry maps a CLI option name (--home, --data, etc.) to a launch context key
      # and a constants.rb constant name. The :inherit flag marks entries with no default
      # constant in constants.rb; children spawned without such a flag cannot re-derive it,
      # so the parent must explicitly pass it to maintain shared state (e.g., --active-session-dir).
      #
      # @return [Array<Hash>] Array of path flag descriptors with keys:
      #   :option, :key, :constant, and optional :inherit
      OPTIONAL_PATH_FLAGS = [
